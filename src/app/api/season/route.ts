        ...
        const season_info = await sql`  
            ...
            season_info AS (
                SELECT 
                    ...
                    (SELECT COUNT(ws.score) from weekly_score ws, current_year_players p
                    WHERE ws.player_id = p.player_id AND EXTRACT(YEAR FROM ws.week_date) = EXTRACT(YEAR FROM CURRENT_DATE)) AS rounds_played,
                    (SELECT SUM(COALESCE(ww.greens, 0) + COALESCE(ww.skins, 0) + COALESCE(ww.best_ball, 0) + COALESCE(ww.partners, 0) + COALESCE(ww.low_score, 0)) 
                    FROM weekly_winnings ww, current_year_players p
                    WHERE ww.player_id = p.player_id AND EXTRACT(YEAR FROM ww.week_date) = EXTRACT(YEAR FROM CURRENT_DATE)) AS season_pot
                FROM current_year_players p
                LEFT JOIN weekly_score ws
                ON ws.player_id = p.player_id
                AND EXTRACT(YEAR FROM ws.week_date) = EXTRACT(YEAR FROM CURRENT_DATE) 
            )
            SELECT 
                total_players,
                weeks_played,
